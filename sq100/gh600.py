...
class GH600(object):
    ...
    COMMANDS = {
        ...
        # 'setTracks':
        #   '02%(payload)s%(isFirst)s%(trackInfo)s%(from)s%(to)s'
        #   '%(trackpoints)s%(checksum)s',
        'getTracks':
            '0200%(payload)s%(numberOfTracks)s%(trackIds)s%(checksum)s',
        ...
        'setWaypoints':
            '02%(payload)s76%(numberOfWaypoints)s%(waypoints)s%(checksum)s',
    ...
    def __init__(self):
        ...
        # logging http://www.tiawichiresearch.com/?p=31 /
...
#         self.serial.timeout = self.config.getint("serial", "timeout")
        ...
        if self.__class__ is GH600:
            if self.config.has_option("general", "firmware"):
                product = {1: 'GH-615', 2: 'GH-625'}[
                    self.config.getint("general", "firmware")]
            else:
    ...
    def getProductModel(self):
        try:
            ...
            return product
        # no response received, assuming command was not understood => old
        # firmware
    ...
    def getExportFormats(self):
        ...
        for format in glob.glob(Utilities.getAppPrefix("exportTemplates",
                                                       "*.txt")):
    ...
    def getTracklist(self):
        raise NotImplemented(
            'This is an abstract method, please instantiate a subclass')
    ...
    def exportTracks(self, tracks, format=None, path=None, **kwargs):
        ...
        if path is None:
            path = os.path.abspath(
                Utilities.getAppPrefix(self.config.get('export', 'path')))
    ...
    def exportWaypoints(self, waypoints, **kwargs):
        ...
        template = Template.from_file(
            Utilities.getAppPrefix('waypoint_template.txt'))
    ...
    def importWaypoints(self, **kwargs):
        ...
        for waypoint in eval(importedWaypoints):
            waypoints.append(Waypoint(str(waypoint['latitude']),
    ...
    def setWaypoints(self, waypoints):
        ...
        checksum = Utilities.checkersum("%s76%s%s" % (
            str(payload), str(numberOfWaypoints), waypointsConverted))

        response = self._querySerial(
            'setWaypoints', **{
                'payload': payload,
    ...
    def getNmea(self):
        ...
        looks interesting
        http://twistedmatrix.com/trac/browser/trunk/twisted/protocols/gps/
               nmea.py
        """
        ...
        while not line.startswith("$GPGGA"):
            self.logger.debug("waiting at serialport: %i" %
        ...
        tokens = line.split(",")
        # [2] is lat in deg+minutes, [3] is {N|S|W|E}
        lat = dmmm2dec(float(tokens[2]), tokens[3])
        # [4] is long in deg+minutes, [5] is {N|S|W|E}
    ...
    def getUnitInformation(self):
        ...
        if len(response) == 180:
            unit = {
                ...
                'sex': (
                    'male'
                    if (Utilities.hex2chr(response[112:114]) == '\x01')
                    else 'female'),
