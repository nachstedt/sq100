...
class GPXParser:
    ...
    def parseTrack(self, trk):
        ...
        for trkseg in trk.getElementsByTagName('trkseg'):
            ...
            trkpt_time = trkseg.getElementsByTagName(
                'trkpt')[0].getElementsByTagName('time')
            if trkpt_time:
                track.date = datetime.datetime.strptime(
                    trkpt_time.firstChild.data, '%Y-%m-%dT%H:%M:%SZ')
            ...
            for i, trkpt in enumerate(trkseg.getElementsByTagName('trkpt')):
                ...
                if trkpt.getElementsByTagName('ele'):
                    track.altitude = int(
                        float(trkpt.getElementsByTagName(
                            'ele')[0].firstChild.data))

                if trkpt.getElementsByTagName('speed'):
                    trackpoint.speed = int(
                        float(trkpt.getElementsByTagName(
                            'speed')[0].firstChild.data))
                ...
                if trkpt.getElementsByTagName('time'):
                    time = datetime.datetime.strptime(
                        trkpt.getElementsByTagName(
                            'time')[0].firstChild.data, '%Y-%m-%dT%H:%M:%SZ')
                ...
                if i > 0:
                    #     track.distance += self.calcDistance(
                    #     trackpoint.latitude,
                    #     trackpoint.longitude,
                    #     track.trackpoints[
                    #         len(track.trackpoints)-1]['latitude'],
                    #     track.trackpoints[
                    #         len(track.trackpoints)-1]['longitude'])
                    track.distance += 1
