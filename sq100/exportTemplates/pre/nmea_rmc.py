...
def pre(track):
    ...
    for trackpoint in track.trackpoints:
        ...
        trackpoint.checksum = nmeaChecksum(
            '$GPRMC,' + trackpoint.status + ',' +
            str(trackpoint.date.strftime("%H%M%S")) + ',' +
            str(trackpoint.latitude_dms) + ',' +
            str(trackpoint.longitude_dms) + ',' +
            str(trackpoint.speed_knots) + ',' +
            str(trackpoint.angle) + ',' +
            str(trackpoint.date.strftime("%d%m%y")) + ',' +
            trackpoint.magnetic)
