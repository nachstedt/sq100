...
class GH625(GH600):
    GH600.COMMANDS.update({
        'setTracks': '02%(payload)s91%(trackInfo)s%(from)s%(to)s'
                     '%(trackpoints)s%(checksum)s',
        'setTracksLaps': '02%(payload)s90%(trackInfo)s%(laps)s'
                         '%(nrOfTrackpoints)s%(checksum)s'
    })
    ...
    def getTracklist(self):
        ...
        return [
            TrackWithLaps(
                date=datetime.datetime(2000 + track.year, track.month,
                                       track.day, track.hour, track.minute,
                                       track.second),
    ...
    def getTracks(self, trackIds):
        ...
        self._writeSerial('getTracks', **{'payload': payload,
        ...
        while True:
            ...
            if data != '8A000000':
                ...
                if data[60:64] == "FFFF":
                    ...
                    self.logger.debug('adding trackpoints %i-%i of %i' % (
                        Utilities.hex2dec(data[60:64]),
                        Utilities.hex2dec(data[64:68]),
                        track.trackpointCount))
    ...
    def setTracks(self, tracks):
        for track in tracks:
            ...
            if response == '91000000' or response == '90000000':
                self.logger.info(
                    'uploaded lap information of track successfully')
            else:
            ...
            for i, chunk in enumerate(trackpointChunks):
                ...
                if response == '9A000000':
                    ...
                    self.logger.debug("uploaded chunk %i of %i" % (
                        i + 1, len(trackpointChunks)))
                elif response == '92000000':
                    # this probably means segment was not as expected, resend
                    # previous segment?
