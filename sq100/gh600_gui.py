...
class Waypoints:
    ...
    def getWaypointsFromGh(self, force=False):
        if "waypoints" not in cherrypy.session or force:
    ...
    def index(self):
        ...
        return t.render(waypointLibrary=waypointLibrary,
                        remoteWaypoints=remoteWaypoints)
    ...
    @cherrypy.expose
    def doAdd(self, latitude, longitude, altitude, title, type=0,
              operation="save"):
    ...
    def batch(self):
        ...
        waypointLibrary = self.getWaypointLibrary()
        return t.render(waypointLibrary=waypointLibrary,
                        waypointTypes=Waypoint.TYPES)

    @cherrypy.expose
    def doBatch(self, latitude, longitude, altitude, title, type,
                operation="save"):
        waypoints = []
        if isinstance(latitude, str):
            waypoints.append(
                Waypoint(latitude, longitude, altitude, title, type))
        else:
            for latitude, longitude, altitude, title, type in zip(
                    latitude, longitude, altitude, title, type):
                waypoints.append(
                    Waypoint(latitude, longitude, altitude, title, type))
        self.saveWaypointLibrary(waypoints)
    ...
    def getWaypointAltitude(self, latitude=0, longitude=0):
        try:
            f = urllib.urlopen("http://ws.geonames.org/srtm3?lat=%s&lng=%s" % (
